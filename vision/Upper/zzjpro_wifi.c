...
uchar matrixkeyscan()            //çŸ©é˜µé”®ç›˜æ‰«æå‡½æ•°
{
  unsigned char temp=0;
  unsigned char key=0;   
  unsigned int rnum;
  unsigned int cnum;  
  P3=0xf0+(rline&0x0f);       //å°†æŸè¡Œçº¿ç½®ä½Žç”µå¹³
  temp=P3;
  temp=temp&0xf0;
  if(temp!=0xf0)
  {
    ...
    if(temp!=0xf0)
    {
      cnum=bitnum(temp/16);   //å¾—åˆ°åˆ—çº¿æ•°å€¼
      temp=P3;
      temp=temp&0x0f;
      rnum=bitnum(temp);      //å¾—åˆ°è¡Œçº¿æ•°å€¼
      key=(rnum-1)*4+cnum-1;  //è®¡ç®—å¾—åˆ°keyå€¼
      temp=P3;
      return key;
    }
  } 
...
void grab(bit dir)  //0å¼€ï¼Œ1é—­
{
	in1=dir,in2=~dir;
}

void pump(uchar sta)//0å¼€ï¼Œ1å…³
{
  relay=sta;		
}
